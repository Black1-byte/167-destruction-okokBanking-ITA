...
lib.callback.register('okokBanking:GetPlayerName', function(source,cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local data = {name = xPlayer.getName()}
    return data or 0
end)
...
RegisterServerEvent("okokBanking:GiveCard")
AddEventHandler("okokBanking:GiveCard", function(amount)
	local xPlayer = ESX.GetPlayerFromId(source)

	if Config.CreditCardCost <= xPlayer.getAccount('bank').money then
		xPlayer.removeAccountMoney('bank', amount)
		TriggerEvent('okokBanking:AddTransferTransactionToSociety', amount, source, "bank", "Bank (IBAN)")
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Ti sono stati addebitati "..Config.IBANChangeCost.."€ per il ritiro della carta di", 5000, 'success')
		if ox_inventory:CanCarryItem(source, 'credit', 1) then
			meta = {type = 'di '.. xPlayer.getName()}
			ox_inventory:AddItem(source, 'credit', 1, meta, nil, function(success, reason)
				
			end)
		else
			TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", 'Non hai abbastanza spazio nell\'inventario.', 5000, 'error')
		end	
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Devi avere "..Config.IBANChangeCost.."€ per ritirare la carta di credito", 5000, 'error')
	end
...
AddEventHandler("okokBanking:DepositMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai depositato "..amount.."€", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Non hai abbastanza contanti", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoney", function(amount)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai prelevato "..amount.."€", 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Non hai abbastanza soldi in banca", 5000, 'error')
	end
end)
	...
	if xPlayer.identifier ~= targetIdentifier then
		if amount <= playerMoney then
			...
			if xTarget ~= nil then
				...
				for i=1, #xPlayers, 1 do
				    ...
				    if xForPlayer.identifier == targetIdentifier then
						...
						TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai ricevuto "..amount.."€ da "..xPlayer.getName(), 5000, 'success')
				    end
				end
				TriggerEvent('okokBanking:AddTransferTransaction', amount, xTarget, source)
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai inviato "..amount.."€ a "..xTarget.getName(), 5000, 'success')
			elseif xTarget == nil then
				...
				TriggerClientEvent('okokBanking:updateTransactions', source, xPlayer.getAccount('bank').money, xPlayer.getMoney())
				TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai inviato "..amount.."€ a "..targetName, 5000, 'success')
		...
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Non puoi inviare soldi a te stesso", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:DepositMoneyToSociety", function(amount, society, societyName)
	...
	if amount <= playerMoney then
		...
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai depositato "..amount.."€ a "..societyName, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Non hai abbastanza contanti", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:WithdrawMoneyToSociety", function(amount, society, societyName, societyMoney)
	...
	if amount <= db.value then
		if db.is_withdrawing == 1 then
			TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Qualcuno sta gia ritirando", 5000, 'error')
		else
			...
			TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai prelevato "..amount.."€ da "..societyName, 5000, 'success')
		end
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "La tua società non ha abbastanza soldi in banca", 5000, 'error')
	end
...
AddEventHandler("okokBanking:TransferMoneyToSociety", function(amount, ibanNumber, societyName, society)
	...
	local playerMoney = xPlayer.getAccount('bank').money
		...
		if amount <= playerMoney then
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai inviato "..amount.."€ a "..societyName, 5000, 'success')
		else
			TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Non hai abbastanza soldi in banca", 5000, 'error')
		end
end)
...
AddEventHandler("okokBanking:TransferMoneyToSocietyFromSociety", function(amount, ibanNumber, societyNameTarget, societyTarget, society, societyName, societyMoney)
	...
	if amount <= societyMoney then
		...
		TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai trasferito "..amount.."€ a "..societyNameTarget, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK",  "La tua società non ha abbastanza soldi in banca", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:TransferMoneyToPlayerFromSociety", function(amount, ibanNumber, targetIdentifier, acc, targetName, society, societyName, societyMoney, toMyself)
	...
	if amount <= societyMoney then
		...
		if xTarget ~= nil then
			...
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai trasferito "..amount.."€ a "..xTarget.getName(), 5000, 'success')
		elseif xTarget == nil then
			local playerAccount = json.decode(acc)
			playerAccount.bank = playerAccount.bank + amount
			playerAccount = json.encode(playerAccount)
			...
			TriggerEvent('okokBanking:AddTransferTransactionFromSocietyToP', amount, society, societyName, targetIdentifier, targetName)
			TriggerClientEvent('okokBanking:updateTransactionsSociety', source, xPlayer.getMoney())
			TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Hai trasferito"..amount.."€ a "..targetName, 5000, 'success')
...
AddEventHandler("okokBanking:AddWithdrawTransaction", function(amount, source_)
	...
	MySQL.Async.insert('INSERT INTO okokBanking_transactions (receiver_identifier, receiver_name, sender_identifier, sender_name, date, value, type) VALUES (@receiver_identifier, @receiver_name, @sender_identifier, @sender_name, CURRENT_TIMESTAMP(), @value, @type)', {
		...
		['@sender_identifier'] = 'Banca',
		['@sender_name'] = 'Account Banca',
		['@value'] = tonumber(amount),
		['@type'] = 'Ritirare'
	}, function (result)
	end)
...
AddEventHandler("okokBanking:UpdateIbanDB", function(iban, amount)
	...
	if Config.IBANChangeCost <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "IBAN cambiato: "..iban, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Devi avere "..Config.IBANChangeCost.."€ per cambiare il tuo IBAN", 5000, 'error')
	end
end)
...
AddEventHandler("okokBanking:UpdatePINDB", function(pin, amount)
	...
	if Config.PINChangeCost <= xPlayer.getAccount('bank').money then
		xPlayer.removeAccountMoney('bank', amount)
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Ti sono stati tolti "..Config.PINChangeCost.."€ per aver cambiato il tuo PIN", 5000, 'success')
		...
		TriggerClientEvent('okokBanking:updateIbanPinChange', source)
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "PIN cambiato in: "..pin, 5000, 'success')
	else
		TriggerClientEvent('okokNotify:Alert', source, "ANTHEM BANK", "Devi avere "..Config.PINChangeCost.."€ per cambiare il tuo PIN", 5000, 'error')
	end
end)
